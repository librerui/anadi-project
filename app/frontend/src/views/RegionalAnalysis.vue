<template>
  <div>
      ...
      <template #content>
        <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 1rem;">
          <div style="display: flex; flex-direction: column; gap: 0.25rem; min-width: 0;">
            ...
            <Dropdown id="profile" v-model="form.profile" :options="profiles" :placeholder="t('regional.select_profile')" style="width: 100%;" />
          </div>
          <div style="display: flex; flex-direction: column; gap: 0.25rem; min-width: 0;">
            <label for="version" style="font-weight: 500;">{{ $t('regional.version') }}</label>
            <InputText id="version" v-model="form.version" :placeholder="t('regional.optional_placeholder')" style="width: 100%;" />
          </div>
          <div style="display: flex; flex-direction: column; gap: 0.25rem; min-width: 0;">
    ...
    <Card style="margin-bottom: 1rem;">
      ...
      <template #content>
        <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 1rem;">
          <div style="display: flex; flex-direction: column; gap: 0.25rem; min-width: 0;">
            ...
            <Dropdown
              ...
              :options="districts"
              :placeholder="t('regional.select_district')"
              style="width: 100%;"
          ...
          <div style="display: flex; flex-direction: column; gap: 0.25rem; min-width: 0;">
            ...
            <Dropdown
              ...
              :options="concelhos"
              :placeholder="t('regional.select_municipality')"
              style="width: 100%;"
        ...
        <div v-if="regionPTDs.length > 0" style="margin-top: 1rem; padding: 0.75rem 1rem; background: var(--p-surface-100); border-radius: 0.5rem; display: flex; gap: 1.5rem; flex-wrap: wrap; font-size: 0.875rem;">
          <div>
            <span style="color: var(--p-text-muted-color);">{{ $t('regional.ptds_found') }}:</span>
            <span style="font-weight: 600; margin-left: 0.5rem;">{{ regionPTDs.length }}</span>
          </div>
          <div>
            <span style="color: var(--p-text-muted-color);">{{ $t('regional.district') }}:</span>
            <span style="font-weight: 600; margin-left: 0.5rem;">{{ selectedDistrito || '—' }}</span>
          </div>
          <div>
            <span style="color: var(--p-text-muted-color);">{{ $t('regional.municipality') }}:</span>
            <span style="font-weight: 600; margin-left: 0.5rem;">{{ selectedConcelho || '—' }}</span>
          </div>
        </div>
      </template>
    </Card>
    ...
    <Card style="margin-bottom: 1rem;">
      ...
      <template #content>
        <p style="margin: 0 0 0.75rem 0; font-weight: 500; font-size: 0.875rem;">{{ $t('regional.charger_model') }}</p>
        ...
        <template #content>
          <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(140px, 1fr)); gap: 1rem; margin-bottom: 1.5rem;">
            <div style="background: var(--p-surface-100); padding: 1rem; border-radius: 0.5rem; text-align: center; min-width: 0;">
              <p style="font-size: 0.875rem; color: var(--p-text-muted-color); margin-bottom: 0.25rem;">{{ $t('regional.total_ptds') }}</p>
              <p style="font-size: 1.5rem; font-weight: bold;">{{ results.length }}</p>
            </div>
            <div style="background: var(--p-surface-100); padding: 1rem; border-radius: 0.5rem; text-align: center; min-width: 0;">
              <p style="font-size: 0.875rem; color: var(--p-text-muted-color); margin-bottom: 0.25rem;">{{ $t('regional.low_risk') }}</p>
              <p style="font-size: 1.5rem; font-weight: bold; color: #22c55e;">{{ summaryCounts.baixo }}</p>
            </div>
            <div style="background: var(--p-surface-100); padding: 1rem; border-radius: 0.5rem; text-align: center; min-width: 0;">
              <p style="font-size: 0.875rem; color: var(--p-text-muted-color); margin-bottom: 0.25rem;">{{ $t('regional.medium_risk') }}</p>
              <p style="font-size: 1.5rem; font-weight: bold; color: #f97316;">{{ summaryCounts.medio }}</p>
            </div>
            <div style="background: var(--p-surface-100); padding: 1rem; border-radius: 0.5rem; text-align: center; min-width: 0;">
              <p style="font-size: 0.875rem; color: var(--p-text-muted-color); margin-bottom: 0.25rem;">{{ $t('regional.high_risk') }}</p>
              <p style="font-size: 1.5rem; font-weight: bold; color: #ef4444;">{{ summaryCounts.alto }}</p>
            </div>
            <div style="background: var(--p-surface-100); padding: 1rem; border-radius: 0.5rem; text-align: center; min-width: 0;">
              <p style="font-size: 0.875rem; color: var(--p-text-muted-color); margin-bottom: 0.25rem;">{{ $t('regional.avg_prediction') }}</p>
              <p style="font-size: 1.5rem; font-weight: bold;">{{ avgPrediction.toFixed(2) }} {{ $t('regional.units.kw') }}</p>
            </div>
          </div>
          ...
          <div style="display: grid; gap: 1rem; grid-template-columns: repeat(auto-fit, minmax(280px, 1fr)); align-items: start;">
            ...
            <div style="display: flex; flex-direction: column; gap: 0.75rem;">
              <div style="background: var(--p-surface-100); padding: 1rem; border-radius: 0.75rem;">
                ...
                <div style="display: flex; flex-direction: column; gap: 0.4rem; font-size: 0.8rem; color: var(--p-text-muted-color);">
                  <div style="display: flex; align-items: center; gap: 0.5rem;">
                    ...
                    <span>{{ $t('regional.legend_baixo') }} ({{ summaryCounts.baixo }})</span>
                  </div>
                  <div style="display: flex; align-items: center; gap: 0.5rem;">
                    <span style="width: 12px; height: 12px; border-radius: 50%; background: #f97316; display: inline-block;"></span>
                    <span>{{ $t('regional.legend_medio') }} ({{ summaryCounts.medio }})</span>
                  </div>
                  <div style="display: flex; align-items: center; gap: 0.5rem;">
                    <span style="width: 12px; height: 12px; border-radius: 50%; background: #ef4444; display: inline-block;"></span>
                    <span>{{ $t('regional.legend_alto') }} ({{ summaryCounts.alto }})</span>
                  </div>
                </div>
              </div>
              ...
              <div style="background: var(--p-surface-100); padding: 1rem; border-radius: 0.75rem;">
                ...
                <p style="margin: 0; font-size: 0.875rem; color: var(--p-text-muted-color);">
                  {{ mapResults.length }} {{ $t('regional.map_points') }}
                </p>
              </div>
            </div>
          </div>
      ...
      <Card>
        ...
        <template #content>
          <DataTable
            ...
            :globalFilterFields="['ptd.codigo_instalacao', 'ptd.distrito', 'ptd.concelho', 'classification']"
          >
            <template #header>
            ...
            <Column field="ptd.codigo_instalacao" :header="$t('regional.ptd_code')" sortable style="min-width: 10rem">
              <template #body="{ data }">
                <span style="font-weight: 600; font-family: monospace;">{{ data.ptd.codigo_instalacao }}</span>
              </template>
            </Column>
            ...
            <Column field="prediction" :header="$t('regional.predicted_load')" sortable style="min-width: 10rem">
              <template #body="{ data }">
                <span style="font-weight: 600;">{{ Number(data.prediction).toFixed(2) }} {{ $t('regional.units.kw') }}</span>
              </template>
            </Column>

            <Column field="ptd.potencia_instalada" :header="$t('regional.installed_power')" sortable style="min-width: 10rem">
              <template #body="{ data }">
                {{ data.ptd.potencia_instalada }} {{ $t('regional.units.kva') }}
              </template>
            </Column>

            <Column field="totalLoad" :header="$t('regional.total_load')" sortable style="min-width: 10rem">
              <template #body="{ data }">
                <span :style="{ fontWeight: 600, color: data.totalLoad > data.ptd.potencia_instalada ? '#ef4444' : 'inherit' }">
                  {{ data.totalLoad.toFixed(2) }} {{ $t('regional.units.kw') }}
                </span>
              </template>
            </Column>
            ...
            <Column field="margin" :header="$t('regional.margin')" sortable style="min-width: 10rem">
              <template #body="{ data }">
                <span :style="{ fontWeight: 600, color: getMarginColor(data.margin) }">
            ...
            <Column field="classification" :header="$t('regional.classification')" sortable style="min-width: 10rem">
              <template #body="{ data }">
                <Tag
                  :value="overloadClassLabel(data.classification)"
                  :severity="classSeverity(data.classification)"
            ...
            <Column field="supportedChargers" :header="$t('regional.supported_chargers')" sortable style="min-width: 10rem">
              <template #body="{ data }">
                <span style="font-weight: 600;">{{ data.supportedChargers }}</span>
              </template>
            </Column>

            <Column :header="$t('regional.actions')" style="min-width: 8rem">
              <template #body="{ data }">
                <Button icon="pi pi-search" outlined rounded size="small" @click="viewPTDDetails(data)" />
              </template>
            </Column>
          </DataTable>
        </template>
...
const canAnalyse = computed(() =>
  !!selectedDistrito.value && regionPTDs.value.length > 0 && !analysing.value
)

const analysisLabel = computed(() => {
  const count = regionPTDs.value.length
  return t('regional.run_analysis', { count })
})
...
const distributionChartData = computed(() => {
  ...
  return {
    ...
    datasets: [{
      data: [counts.baixo, counts.medio, counts.alto],
...
const mapResults = computed(() =>
  results.value.filter(r =>
    typeof r.ptd.latitude === 'number' && typeof r.ptd.longitude === 'number'
  )
)
...
const loadPTDs = async () => {
  try {
    ...
    toast.add({
      ...
      detail: error.message || 'Failed to load PTDs',
      life: 5000,
    })
  }
}
...
const onConcelhoChange = () => {
  // Região alterada, limpar resultados anteriores
  results.value = []
}
...
const runRegionalAnalysis = async () => {
  ...
  for (let i = 0; i < ptds.length; i += batchSize) {
    ...
    const batchPromises = batch.map(async (ptd) => {
      ...
      try {
        ...
        const supportedChargers = singleChargerEffectiveLoad > 0
          ? Math.max(0, Math.floor(capacity / singleChargerEffectiveLoad))
          : 0
        ...
        // Em caso de erro, retornar com valores defaults
        return {
          ptd,
          prediction: 0,
          totalLoad: totalChargerLoad.value,
          margin: -1,
  ...
  toast.add({
    ...
    detail: t('regional.analysis_complete', { count: newResults.length }),
    life: 3000,
  })
}
  ...
  validResults.forEach((result) => {
    const popupContent = `
      <div style="min-width: 200px;">
        ...
        <div style="margin-top: 0.5rem; padding: 0.25rem 0.5rem; border-radius: 0.25rem; background: ${getClassificationColor(result.classification)}22; color: ${getClassificationColor(result.classification)}; font-weight: 600; display: inline-block;">
          ${overloadClassLabel(result.classification)}
        </div>
      </div>
    `

    const marker = L.circleMarker(
      [result.ptd.latitude as number, result.ptd.longitude as number],
      {
        radius: 6,
        fillColor: getClassificationColor(result.classification),
        color: '#ffffff',
        weight: 2,
        opacity: 1,
        fillOpacity: 0.9,
      ...
      .on('click', () => {
        // Opcional: scroll para a linha da tabela
      })
...
// ── Tabela ──
const clearFilters = () => {
  // DataTable filters são limpos via ref se necessário
}
...
const exportCSV = () => {
  const headers = [
    t('regional.ptd_code'),
    t('regional.district'),
    t('regional.municipality'),
    t('regional.predicted_load'),
    t('regional.installed_power'),
    t('regional.total_load'),
    t('regional.margin'),
    t('regional.classification'),
    t('regional.supported_chargers'),
  ]

  const rows = results.value.map(r => [
    r.ptd.codigo_instalacao,
    r.ptd.distrito,
    r.ptd.concelho,
    r.prediction.toFixed(2),
    r.ptd.potencia_instalada,
    r.totalLoad.toFixed(2),
    (r.margin * 100).toFixed(1) + '%',
    overloadClassLabel(r.classification),
    r.supportedChargers,
  ])
...
const viewPTDDetails = (result: RegionalResult) => {
  // Opcional: abrir modal ou navegar para detalhes do PTD
  toast.add({
    severity: 'info',
    summary: result.ptd.codigo_instalacao,
    detail: `${t('regional.predicted_load')}: ${result.prediction.toFixed(2)} kW | ${t('regional.classification')}: ${overloadClassLabel(result.classification)}`,
    life: 5000,
  })
}
...
// Watch para re-renderizar mapa quando resultados mudam
